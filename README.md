...
Nested paths in field and column parameters use `.` as their delimiter by default. Set `--field-delimiter` to use another string. This is useful when a field name contains a literal dot: for example, `--field-delimiter=/ --column-key 'user.info/ssn=KEY'` addresses the nested field `ssn` under a field literally named `user.info`. The delimiter applies to `--column-key`, `--writer-column-key`, `--field-encoding`, `--field-compression`, `--field-bloom-filter`, and column paths in key files.
...
$ parquet-tools transcode -s input.parquet \
    --field-compression shoe_brand=GZIP \
    --field-compression shoe_name=ZSTD \
    --compression-level GZIP=9,ZSTD=3 \
    output.parquet
```
